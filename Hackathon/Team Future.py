# -*- coding: utf-8 -*-
"""
Created on Sun Nov 18 08:35:38 2018
...
class Life2Coding(QtWidgets.QMainWindow):
    def __init__(self):
        super(Life2Coding,self).__init__()
        loadUi("Main_Window.ui",self)
        self.Btn_Vid1.clicked.connect(self.start_vid)
        self.Btn_Vid2.clicked.connect(self.start_vid2)
    ...
    def start_vid2(self):
                        ...
                        for i in persons:
                            if abs(cx-i.getX()) <= w and abs(cy-i.getY()) <= h:
                                ...
                                if i.going_UP(line_down,line_up) == True:
                                    ...
                                    accessKey = "8021400325e413c574ad"
                                    secretKey = "1e80bf4af278f41575dc92d6387291b230acb6a8"
                                    ...
                                    #cv2.rectangle(frame,(content['licensePlates'][0]['boundingBox']['x'],content['licensePlates'][0]['boundingBox']['y']),(content['licensePlates'][0]['boundingBox']['x']+content['licensePlates'][0]['boundingBox']['w'],content['licensePlates'][0]['boundingBox']['y']+content['licensePlates'][0]['boundingBox']['h']),(0,255,0),2)
                                    print(content)
                                    str_up += 'Car '+str(e)+': '+ (content['licensePlates'][0]['licensePlateNumber'] + "  ")
                                    e += 1
        ...
        #    str_down = 'DOWN: '+ str(cnt_down)
        #    frame = cv2.polylines(frame,[pts_L1],False,line_down_color,thickness=2)
            frame = cv2.polylines(frame,[pts_L2],False,line_up_color,thickness=5)
        #    frame = cv2.polylines(frame,[pts_L3],False,(255,255,255),thickness=1)
        #    frame = cv2.polylines(frame,[pts_L4],False,(255,255,255),thickness=1)
            cv2.putText(frame, str_up ,(10,40),font,0.5,(255,255,255),2,cv2.LINE_AA)
            cv2.putText(frame, str_up ,(10,40),font,0.5,(0,0,255),1,cv2.LINE_AA)
...
if __name__ == '__main__' :
    ...
    window = Life2Coding()
    window.setWindowTitle('Smart Traffic Light System')
    window.show()
    sys.exit(app.exec_())